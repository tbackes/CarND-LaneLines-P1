...
- My method doesn't use hough lines, so I cannot filter on what angle/orientation of line segments to include before calculating my best fit line. This means that if large white/yellow objects appear in an image they may have a large influence on my calculated lane line. For example, if there was a turn arrow or many horizontal dashes in the lane I have no way of filtering out those pixels beyond my coarse image mask from step 2 of the pipeline.
